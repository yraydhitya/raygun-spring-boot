 ...
 * maximum number of capacity. This event has a tiny probability for it to happen.
 *
...
class RaygunTemplateMessagesRejectionTest {
  ...
  void asyncCouldRejectMessagesWhenQueueIsFull() throws InterruptedException {
    RaygunClient raygunClient = new DelayedMockRaygunClient(100);
    Thread raygunThread = raygunThread(raygunClient, threadPoolTaskExecutor());
    raygunThread.start();
    raygunThread.join();
  }

  Thread raygunThread(RaygunClient raygunClient, TaskExecutor taskExecutor) {
    RaygunClientFactory raygunClientFactory = mock(RaygunClientFactory.class);
    when(raygunClientFactory.newClient()).thenReturn(raygunClient);
    RaygunTemplate raygunTemplate = new RaygunTemplate(raygunClientFactory, taskExecutor);
    return new Thread(new RaygunRunnable(raygunTemplate, 200));
  }
